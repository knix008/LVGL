...
static lv_font_t *korean_font_24 = NULL;
...
static void close_popup_cb(lv_event_t *e)
{
    ...
    lv_obj_t *overlay = lv_obj_get_parent(popup);
    lv_obj_del(overlay);  // Delete the overlay (which also deletes the popup)
}
...
static void button_matrix_cb(lv_event_t *e)
{
    ...
    if (strcmp(text, "C") == 0 || strcmp(text, "지우기") == 0) {
        ...
        if (buffer_index > 0) {
            ...
            lv_obj_set_style_bg_opa(overlay, LV_OPA_TRANSP, 0);  // Fully transparent overlay
            lv_obj_set_style_border_width(overlay, 0, 0);
            lv_obj_clear_flag(overlay, LV_OBJ_FLAG_SCROLLABLE);

            // Create custom popup dialog with Korean font support
            lv_obj_t *mbox = lv_obj_create(overlay);
            lv_obj_set_size(mbox, 270, 280);  // Dialog size for image and content
            lv_obj_align(mbox, LV_ALIGN_BOTTOM_MID, 0, -20);  // Position at bottom
            lv_obj_set_style_bg_color(mbox, lv_color_hex(0x1A1A1A), 0);  // Dark gray/black background
            lv_obj_set_style_bg_opa(mbox, LV_OPA_60, 0);  // 60% opaque for more transparency
            lv_obj_set_style_border_width(mbox, 2, 0);
            lv_obj_set_style_border_color(mbox, lv_color_hex(0x404040), 0);  // Dark gray border
            lv_obj_set_style_radius(mbox, 10, 0);
            lv_obj_set_style_shadow_width(mbox, 20, 0);  // Add shadow for depth
            ...
            // Create PNG image
            lv_obj_t *img = lv_img_create(mbox);
            lv_img_set_src(img, "A:buttons/Button-Info-icon-resized.png");
            lv_obj_set_width(img, 80);
            lv_obj_set_height(img, 80);
            lv_obj_align(img, LV_ALIGN_TOP_MID, 0, 10);
            
            // Create title label
            lv_obj_t *title_label = lv_label_create(mbox);
            lv_label_set_text(title_label, "숫자 입력");
            lv_obj_align(title_label, LV_ALIGN_TOP_MID, 0, 100);  // Position below image (100px from top)
            lv_obj_set_style_text_color(title_label, lv_color_hex(0xFFFFFF), 0);  // White text for contrast
            if (korean_font_16) {
            ...
            lv_obj_align(content_label, LV_ALIGN_TOP_MID, 0, 135);  // Position below title
            lv_obj_set_style_text_color(content_label, lv_color_hex(0xFFFFFF), 0);  // White text for contrast
            if (korean_font_16) {
            ...
            lv_obj_set_style_bg_color(close_btn, lv_color_hex(0x00AA00), 0);  // Green button
            lv_obj_set_style_bg_opa(close_btn, LV_OPA_80, 0);  // Semi-transparent button
            lv_obj_t *close_label = lv_label_create(close_btn);
            lv_label_set_text(close_label, "닫기");
            lv_obj_align(close_label, LV_ALIGN_CENTER, 0, 0);
            lv_obj_set_style_text_color(close_label, lv_color_hex(0xFFFFFF), 0);  // White text
            if (korean_font_16) {
...
static void init_korean_fonts(void)
{
    ...
    // Create font info structure for size 24 (for larger buttons)
    static lv_ft_info_t info_24 = {0};
    info_24.name = "fonts/NanumGothicCoding-Bold.ttf";
    info_24.weight = 20;
    info_24.style = FT_FONT_STYLE_NORMAL;

    // Load NanumGothicCoding-Bold font at size 24 (for buttons)
    if (!lv_ft_font_init(&info_24)) {
        fprintf(stderr, "Failed to load Korean font at size 24!\n");
    } else {
        korean_font_24 = info_24.font;
        printf("Korean font (size 24) loaded successfully!\n");
    }
}
...
static void create_number_input_ui(void)
{
    ...
    lv_obj_set_style_text_font(display_label, korean_font_20, 0);
    ...
    // Apply Korean font (size 20) to button matrix
    if (korean_font_20) {
        lv_obj_set_style_text_font(keypad, korean_font_20, LV_PART_ITEMS);
    }
