...
void create_korean_keypad_ui(lv_obj_t * parent) {
    ...
    // Set light background for the entire keypad to make buttons more visible
    lv_obj_set_style_bg_color(parent, lv_color_hex(0xF5F5F5), 0);
    lv_obj_set_style_bg_opa(parent, LV_OPA_COVER, 0);
    
    printf("Creating keypad title...\n");
    // Keypad title
    ...
    lv_obj_set_style_text_color(keypad_label, lv_color_hex(0x212121), 0);
    if (korean_font != NULL) {
        lv_obj_set_style_text_font(keypad_label, korean_font, 0);
    } else {
        printf("Warning: Korean font is NULL for title\n");
    }
    lv_obj_align(keypad_label, LV_ALIGN_TOP_MID, 0, 10);

    printf("Creating display area...\n");
    ...
    lv_obj_set_style_bg_color(keypad_display, lv_color_hex(0xFFFFFF), 0);
    ...
    lv_obj_set_style_text_color(keypad_display, lv_color_hex(0x212121), 0);
    ...
    if (korean_font != NULL) {
        ...
        printf("Warning: Korean font is NULL for mode button\n");
    }
    ...
    // Add debug info
    printf("Hangul buttons creation completed\n");

    printf("Creating function buttons...\n");
    // Function buttons row
    int btn_width = 65;
    int btn_height = 38;
    int start_x = 15;
    int start_y = 380; // Moved up to be closer to vowel buttons
...
static void create_hangul_buttons(lv_obj_t * parent) {
    ...
    printf("Starting to create Hangul buttons...\n");
    ...
    printf("Creating consonants (자음)...\n");
    // 자음(19개): 10개씩 2줄
    int jaum_per_row = 10;
    int jaum_count = 19;
    for (int i = 0; i < jaum_count; i++) {
        int row = i / jaum_per_row;
        int col = i % jaum_per_row;
        lv_obj_t * btn = lv_btn_create(parent);
        if (btn == NULL) {
            printf("Error: Failed to create consonant button %d\n", i);
            continue;
        }
        lv_obj_set_size(btn, btn_width, btn_height);
        lv_obj_align(btn, LV_ALIGN_TOP_LEFT, start_x + col * (btn_width + gap), start_y + row * (btn_height + gap));
        lv_obj_set_style_bg_color(btn, lv_color_hex(0x2196F3), 0);
        lv_obj_set_style_text_color(btn, lv_color_hex(0xFFFFFF), 0);
        lv_obj_set_style_border_color(btn, lv_color_hex(0x1976D2), 0);
        lv_obj_set_style_border_width(btn, 2, 0);
        lv_obj_t * btn_label = lv_label_create(btn);
        if (btn_label == NULL) {
            printf("Error: Failed to create consonant label %d\n", i);
            continue;
        }
        ...
        if (korean_font != NULL) {
            ...
            printf("Warning: Korean font is NULL for consonant %d, using default font\n", i);
    ...
    printf("Creating vowels (모음)...\n");
    // 모음(21개): 7개씩 3줄
    int moum_per_row = 7;
    int moum_count = 21;
    int moum_start_y = start_y + 2 * (btn_height + gap) + 10;
    for (int i = 0; i < moum_count; i++) {
        int row = i / moum_per_row;
        int col = i % moum_per_row;
        lv_obj_t * btn = lv_btn_create(parent);
        if (btn == NULL) {
            printf("Error: Failed to create vowel button %d\n", i);
            continue;
        }
        lv_obj_set_size(btn, btn_width, btn_height);
        lv_obj_align(btn, LV_ALIGN_TOP_LEFT, start_x + col * (btn_width + gap), moum_start_y + row * (btn_height + gap));
        lv_obj_set_style_bg_color(btn, lv_color_hex(0x4CAF50), 0);
        lv_obj_set_style_text_color(btn, lv_color_hex(0xFFFFFF), 0);
        lv_obj_set_style_border_color(btn, lv_color_hex(0x388E3C), 0);
        lv_obj_set_style_border_width(btn, 2, 0);
        lv_obj_t * btn_label = lv_label_create(btn);
        if (btn_label == NULL) {
            printf("Error: Failed to create vowel label %d\n", i);
            continue;
        }
        ...
        if (korean_font != NULL) {
            ...
            printf("Warning: Korean font is NULL for vowel %d, using default font\n", i);
            // Use default font as fallback
            lv_obj_set_style_text_font(btn_label, LV_FONT_DEFAULT, 0);
        }
        lv_obj_center(btn_label);
    ...
    printf("Hangul buttons created successfully (자음 + 모음 only)\n");
}
