...
### 6. **stream_recognition** - Stream Real-Time Recognition Results
...
**Description:**
Establishes a persistent connection and streams face recognition results in real-time. The connection remains open and continuously sends recognition updates until the client disconnects.

**Example:**
```bash
./socket_client stream_recognition  # (requires implementation in client)
# Or using netcat:
...
**Process:**
1. Sends initial confirmation: `OK:Stream started`
2. Continuously sends recognition updates every 500ms
3. Connection stays open until client disconnects
4. Each update contains current face recognition state
...
Continuous stream (one line per update):
```
FACE:<person_name>:<confidence>:<timestamp>
```
or
```
NO_FACE:<timestamp>
...
- `FACE`: Face recognized
  - `<person_name>`: Name of recognized person (e.g., "A1", "John1")
  - `<confidence>`: Recognition confidence percentage (0-100)
...
FACE:A1:87:1700000123956
NO_FACE:1700000124456
NO_FACE:1700000124956
FACE:B2:92:1700000125456
FACE:B2:91:1700000125956
```
...
**Notes:**
- Requires camera to be running (`camera_on`)
- Recognition must be enabled (trained model loaded)
- Updates sent every 500ms (configurable in server)
- Connection is persistent - client must disconnect to stop stream
- Only one face is reported per update (highest confidence if multiple)
- Timestamp is in milliseconds since Unix epoch

**Error Cases:**
- Connection will close if server stops
- No error if camera not running (will send NO_FACE continuously)
- No error if recognition not enabled (will send NO_FACE continuously)
