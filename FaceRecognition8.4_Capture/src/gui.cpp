...
GUI::GUI(int width, int height)
    : screen_width(width), screen_height(height), is_running(true),
      ...
      status_label(nullptr), info_label(nullptr), flash_overlay(nullptr),
...
GUI::~GUI() {
    ...
    // Free shutter sound
    if (shutter_sound != nullptr) {
        Mix_FreeChunk(static_cast<Mix_Chunk*>(shutter_sound));
        shutter_sound = nullptr;
        Mix_CloseAudio();
    }
}
...
bool GUI::create_main_screen() {
    ...
    // Create flash overlay (initially hidden) - must be created last to be on top
    flash_overlay = lv_obj_create(main_screen);
    lv_obj_set_size(flash_overlay, screen_width, screen_height);
...
bool GUI::create_buttons() {
    // 4 buttons in 2 rows: 
    // Row 1: Training, Capture
    // Row 2: Camera On, Camera Off
    int button_width = 90;
    int button_height = 40;
    int padding = 5;  // Gap between buttons
    int row_spacing = 5;  // Spacing between rows

    // Calculate total width needed for 2 buttons per row
    int total_width = 2 * button_width + padding;

    // Center the buttons horizontally
    int start_x = (screen_width - total_width) / 2;
    int row1_y = screen_height - 100;  // First row position
    int row2_y = row1_y + button_height + row_spacing;  // Second row position

    // Row 1: Training button
    lv_obj_t* train_btn = lv_btn_create(main_screen);
    lv_obj_set_size(train_btn, button_width, button_height);
    lv_obj_set_pos(train_btn, start_x, row1_y);
    ...
    lv_obj_set_pos(capture_btn, start_x + button_width + padding, row1_y);
...
void GUI::live_stream_start_btn_event_cb(lv_event_t* e) {
    GUI* gui = static_cast<GUI*>(lv_event_get_user_data(e));
    if (gui && gui->on_live_stream_start) {
        gui->on_live_stream_start();
    }
}
...
void GUI::set_live_stream_start_callback(std::function<void()> callback) {
    on_live_stream_start = callback;
}

void GUI::set_live_stream_stop_callback(std::function<void()> callback) {
...
void GUI::flash_timer_cb(lv_timer_t* timer) {
    GUI* gui = static_cast<GUI*>(timer->user_data);
    if (gui && gui->flash_overlay) {
        lv_obj_add_flag(gui->flash_overlay, LV_OBJ_FLAG_HIDDEN);
    }
    lv_timer_del(timer);  // Delete the timer after use
}

// Show white flash overlay for photo capture effect
void GUI::show_flash(uint32_t duration_ms) {
    ...
    lv_timer_t* timer = lv_timer_create(flash_timer_cb, duration_ms, this);
    lv_timer_set_repeat_count(timer, 1);
}

// Play shutter sound effect
void GUI::play_shutter_sound() {
    ...
    if (shutter_sound == nullptr) {
        ...
        Mix_Chunk* sound = Mix_LoadWAV(SHUTTER_SOUND_PATH);
        if (sound == nullptr) {
            std::cerr << "Failed to load shutter sound '" << SHUTTER_SOUND_PATH 
                     << "': " << Mix_GetError() << std::endl;
            return;
        }

        shutter_sound = static_cast<void*>(sound);
        std::cout << "Shutter sound loaded successfully from " << SHUTTER_SOUND_PATH << std::endl;
    }

    // Play the shutter sound on first available channel
    Mix_Chunk* sound = static_cast<Mix_Chunk*>(shutter_sound);
    if (Mix_PlayChannel(-1, sound, 0) == -1) {
        std::cerr << "Failed to play shutter sound: " << Mix_GetError() << std::endl;
    }
}
