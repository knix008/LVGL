...
void calendar_navigate(calendar_date_t* date, int direction, int unit) {
    ...
    switch (unit) {
        case 0: // Day navigation
            if (direction > 0) {
                // Next day
                days_in_month = calendar_get_days_in_month(date->year, date->month);
                date->day++;
...
// Navigate to previous week - DISABLED (using unified navigation)
/*
