...
// Circle button event handler - for image-based buttons
static void circle_button_event_handler(lv_event_t *e)
{
    lv_event_code_t code = lv_event_get_code(e);
    lv_obj_t *btn = lv_event_get_target(e);

    if (code == LV_EVENT_PRESSED) {
        // Get the current image source to determine which button this is
        ...
        // Offset to keep center aligned (size increases from 60 to 63, so offset by -1.5 pixels)
        lv_coord_t offset = -1;  // -1 pixel on each side to keep roughly centered

        // Switch to pressed image and adjust position
        if (strstr(src_str, "circle_red")) {
            lv_img_set_src(btn, "A:assets/images/circle_red_pressed.bmp");
            lv_obj_set_pos(btn, x + offset, y + offset);
        }
        else if (strstr(src_str, "circle_green")) {
            lv_img_set_src(btn, "A:assets/images/circle_green_pressed.bmp");
            lv_obj_set_pos(btn, x + offset, y + offset);
        }
        else if (strstr(src_str, "circle_blue")) {
            lv_img_set_src(btn, "A:assets/images/circle_blue_pressed.bmp");
            lv_obj_set_pos(btn, x + offset, y + offset);
        }
    }
        ...
        lv_coord_t offset = 1;  // Restore by +1 pixel

        // Switch back to normal image and restore position
        if (strstr(src_str, "red")) {
            lv_img_set_src(btn, "A:assets/images/circle_red.bmp");
            lv_obj_set_pos(btn, x + offset, y + offset);
        }
        else if (strstr(src_str, "green")) {
            lv_img_set_src(btn, "A:assets/images/circle_green.bmp");
            lv_obj_set_pos(btn, x + offset, y + offset);
        }
        else if (strstr(src_str, "blue")) {
            lv_img_set_src(btn, "A:assets/images/circle_blue.bmp");
            lv_obj_set_pos(btn, x + offset, y + offset);
        }
    }
...
static void create_buttons(void)
{
    ...
    // ===== CIRCLE BUTTONS SECTION (Circular BMP Images) =====

    // Circle Button 1: Red circular BMP image as clickable button
    lv_obj_t *circle_btn1 = lv_img_create(scr);
    lv_img_set_src(circle_btn1, "A:assets/images/circle_red.bmp");
    lv_obj_set_pos(circle_btn1, 30, 450);
    lv_obj_add_flag(circle_btn1, LV_OBJ_FLAG_CLICKABLE);
    lv_obj_add_event_cb(circle_btn1, circle_button_event_handler, LV_EVENT_ALL, NULL);

    // Circle Button 2: Green circular BMP image as clickable button
    lv_obj_t *circle_btn2 = lv_img_create(scr);
    lv_img_set_src(circle_btn2, "A:assets/images/circle_green.bmp");
    lv_obj_set_pos(circle_btn2, 130, 450);
    lv_obj_add_flag(circle_btn2, LV_OBJ_FLAG_CLICKABLE);
    lv_obj_add_event_cb(circle_btn2, circle_button_event_handler, LV_EVENT_ALL, NULL);

    // Circle Button 3: Blue circular BMP image as clickable button
    lv_obj_t *circle_btn3 = lv_img_create(scr);
    lv_img_set_src(circle_btn3, "A:assets/images/circle_blue.bmp");
    lv_obj_set_pos(circle_btn3, 230, 450);
    lv_obj_add_flag(circle_btn3, LV_OBJ_FLAG_CLICKABLE);
    lv_obj_add_event_cb(circle_btn3, circle_button_event_handler, LV_EVENT_ALL, NULL);
}
