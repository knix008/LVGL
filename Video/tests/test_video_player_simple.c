/**
 * @file test_video_player_simple.c
 * @brief Simple test cases for video player functionality without LVGL
 */
...
static void test_video_player_state_transitions(void) {
    ...
    // Test valid transitions (without file loading)
    video_player_play();
...
static void test_video_player_performance(void) {
    ...
    for (int i = 0; i < 50; i++) {
        video_player_seek(i % 120);
    }
    
    TEST_ASSERT_EQUAL(50, video_player_get_position(), "Position should be 50 after rapid seek operations");
}

// Main test runner
int main(void) {
    printf("========================================\n");
    printf("    Video Player Simple Test Suite\n");
    printf("========================================\n");
