...
// Main program for face detection with video and webcam support
...
void printUsage(const char* programName) {
    std::cout << "Usage: " << programName << " [OPTIONS]\n"
              ...
              << "\nExamples:\n"
              << "  " << programName << " -v ../data/traffic.mp4 -o face_detection_output.mp4\n"
              << "  " << programName << " -w 0 -s -o face_webcam_output.mp4\n"
...
int main(int argc, char** argv) {
    ...
    std::string modelPath = "models/yolov8_face_model.pt";
    ...
    if (saveOutput && outputPath.empty()) {
        if (useWebcam) {
            outputPath = "face_webcam_output.mp4";
        } else {
            // Generate output path from input video path
            size_t lastDot = videoPath.find_last_of('.');
            if (lastDot != std::string::npos) {
                outputPath = videoPath.substr(0, lastDot) + "_face_detected.mp4";
            } else {
                outputPath = videoPath + "_face_detected.mp4";
            }
        }
    }

    std::cout << "YOLOv8 Face Detection" << std::endl;
    std::cout << "======================" << std::endl;
    ...
    try {
        // Initialize face video detector
        YOLOFaceVideoDetector detector(modelPath, cv::Size(640, 640), useCuda);

        bool success = false;
        
        if (useWebcam) {
            std::cout << "Starting face detection webcam..." << std::endl;
            success = detector.processWebcam(cameraIndex, showFPS, saveOutput, outputPath);
        } else {
            std::cout << "Starting face detection video processing..." << std::endl;
            success = detector.processVideoFile(videoPath, outputPath, showFPS, saveOutput);
        }

        if (success) {
            std::cout << "\nFace detection completed successfully!" << std::endl;
            ...
            std::cerr << "Face detection failed!" << std::endl;
            return 1;
        }
