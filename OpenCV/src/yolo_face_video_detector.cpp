...
#include "yolo_face_video_detector.h"
#include <iostream>
#include <iomanip>

YOLOFaceVideoDetector::YOLOFaceVideoDetector(const std::string &modelPath, 
                                             const cv::Size &modelInputShape, 
                                             const bool &runWithCuda)
    : detector(modelPath, modelInputShape, runWithCuda),
      currentFPS(0.0),
...
bool YOLOFaceVideoDetector::processVideoFile(const std::string &videoPath, 
                                             ...
                                             bool saveOutput)
{
    ...
    std::cout << "\nFace detection video processing completed!" << std::endl;
...
bool YOLOFaceVideoDetector::processWebcam(int cameraIndex, 
                                          bool showFPS, 
                                          bool saveOutput, 
                                          const std::string &outputPath)
{
    ...
    while (true) {
        ...
        // Display frame
        cv::imshow("YOLOv8 Face Detection", outputFrame);
        ...
        if (key == 'q' || key == 27) { // 'q' or ESC
            ...
            // Save current frame
            std::string filename = "face_detection_frame_" + std::to_string(frameCount) + ".jpg";
    ...
    std::cout << "\nFace detection webcam processing completed!" << std::endl;
    std::cout << "Total frames processed: " << totalFramesProcessed << std::endl;
...
VideoDetectionResult YOLOFaceVideoDetector::processFrame(const cv::Mat &frame)
{
    auto startTime = std::chrono::high_resolution_clock::now();
...
cv::Mat YOLOFaceVideoDetector::drawDetections(const cv::Mat &frame, 
                                              ...
                                              bool showFPS)
{
    ...
    if (showFPS) {
        ...
        std::string detectionsText = "Faces: " + std::to_string(detections.size());
...
void YOLOFaceVideoDetector::updateFPS()
{
...
bool YOLOFaceVideoDetector::initializeVideoWriter(const std::string &outputPath, const cv::Size &frameSize)
{
