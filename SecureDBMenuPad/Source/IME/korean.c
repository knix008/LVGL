...
// GUI-specific input handling functions (no terminal dependencies)
...
void handle_enter_gui(char* input_buffer, size_t* input_len, wchar_t* output_buffer) {
    // Clear all buffers for GUI (don't print newline)
    input_buffer[0] = '\0';
    output_buffer[0] = L'\0';
    *input_len = 0;
}
...
void handle_space_gui(char* input_buffer, size_t* input_len, wchar_t* output_buffer) {
    ...
    // Add space to output buffer
    size_t out_len = wcslen(output_buffer);
    if (out_len < MAX_OUTPUT_LEN - 1) {
        output_buffer[out_len] = L' ';
        output_buffer[out_len + 1] = L'\0';
    }
}
...
void process_input_gui(char* input_buffer, size_t* input_len, wchar_t* output_buffer, int ch) {
    if (ch == 0x7f) {  // 백스페이스
        handle_backspace_gui(input_buffer, input_len, output_buffer);
    } else if (ch == '\n' || ch == '\r') {  // Enter key
        handle_enter_gui(input_buffer, input_len, output_buffer);
    } else if (ch == ' ') {  // 공백
        handle_space_gui(input_buffer, input_len, output_buffer);
    } else {
        handle_character_gui(input_buffer, input_len, output_buffer, ch);
    }
}
