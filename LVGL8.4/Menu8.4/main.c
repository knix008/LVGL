...
static ScreenState screen_stack[MAX_SCREENS];
...
static void show_screen(int screen_id);
...
static void create_menu_screen(void) {
    ...
    lv_obj_set_style_bg_color(menu_screen, lv_color_hex(0x2A2A2A), 0);

    // Create title bar with back button
    lv_obj_t *menu_title_bar = lv_obj_create(menu_screen);
    lv_obj_set_size(menu_title_bar, SCREEN_WIDTH, 60);
    lv_obj_align(menu_title_bar, LV_ALIGN_TOP_MID, 0, 0);
    lv_obj_set_style_bg_color(menu_title_bar, lv_color_hex(0x1A1A1A), 0);
    lv_obj_set_style_bg_opa(menu_title_bar, 128, 0);
    lv_obj_set_style_border_width(menu_title_bar, 0, 0);
    lv_obj_set_style_pad_all(menu_title_bar, 5, 0);

    // Add title label
    lv_obj_t *menu_title_label = lv_label_create(menu_title_bar);
    lv_label_set_text(menu_title_label, "홈");
    lv_obj_set_style_text_color(menu_title_label, lv_color_hex(0xFFFFFF), 0);
    if (app_state.font_20) {
        lv_obj_set_style_text_font(menu_title_label, app_state.font_20, 0);
    }
    lv_obj_align(menu_title_label, LV_ALIGN_CENTER, 0, 0);

    // Add back button to title bar
    lv_obj_t *menu_back_btn = lv_btn_create(menu_title_bar);
    lv_obj_set_size(menu_back_btn, 60, 40);
    lv_obj_align(menu_back_btn, LV_ALIGN_LEFT_MID, 5, 0);
    lv_obj_set_style_bg_color(menu_back_btn, lv_color_hex(0x444444), 0);
    lv_obj_set_style_border_width(menu_back_btn, 1, 0);
    lv_obj_set_style_border_color(menu_back_btn, lv_color_hex(0x888888), 0);
    lv_obj_t *back_label = lv_label_create(menu_back_btn);
    lv_label_set_text(back_label, "이전");
    lv_obj_set_style_text_color(back_label, lv_color_hex(0xFFFFFF), 0);
    if (app_state.font_20) {
        lv_obj_set_style_text_font(back_label, app_state.font_20, 0);
    }
    lv_obj_align(back_label, LV_ALIGN_CENTER, 0, 0);
    lv_obj_add_event_cb(menu_back_btn, back_btn_callback, LV_EVENT_CLICKED, NULL);

    // Add content area
    lv_obj_t *menu_content = lv_obj_create(menu_screen);
    lv_obj_set_size(menu_content, SCREEN_WIDTH, SCREEN_HEIGHT - 120);  // Account for title and status bars
    lv_obj_align(menu_content, LV_ALIGN_TOP_MID, 0, 60);
    lv_obj_set_style_bg_color(menu_content, lv_color_hex(0x2A2A2A), 0);
    lv_obj_set_style_border_width(menu_content, 0, 0);

    // Add some menu items as example
    lv_obj_t *item1 = lv_label_create(menu_content);
    lv_label_set_text(item1, "메뉴 항목 1\n메뉴 항목 2\n메뉴 항목 3");
    lv_obj_set_style_text_color(item1, lv_color_hex(0xFFFFFF), 0);
    if (app_state.font_20) {
        lv_obj_set_style_text_font(item1, app_state.font_20, 0);
    }
    lv_obj_align(item1, LV_ALIGN_TOP_MID, 0, 20);

    // Create status bar
    lv_obj_t *menu_status_bar = lv_obj_create(menu_screen);
    lv_obj_set_size(menu_status_bar, SCREEN_WIDTH, 50);
    lv_obj_align(menu_status_bar, LV_ALIGN_BOTTOM_MID, 0, 0);
    lv_obj_set_style_bg_color(menu_status_bar, lv_color_hex(0x1A1A1A), 0);
    lv_obj_set_style_bg_opa(menu_status_bar, 128, 0);
    lv_obj_set_style_border_width(menu_status_bar, 0, 0);
    lv_obj_set_style_pad_all(menu_status_bar, 5, 0);

    // Add to screen stack and load
...
static void create_gui(void) {
    ...
    // Add event callback to Menu button
    lv_obj_add_event_cb(btn1, menu_btn_callback, LV_EVENT_CLICKED, NULL);

    // Button 2 (right side) - Exit button (종료)
