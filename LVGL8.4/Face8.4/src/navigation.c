...
void settings_btn_callback(lv_event_t *e) {
    (void)e;
    if (screen_stack[screen_stack_top].screen_id != SCREEN_FACE) {
        // Navigate using absolute path: clear stack to MENU then go to FACE
        screen_stack_top = 0;  // Reset to MAIN
        show_screen(SCREEN_MENU);  // Go through MENU
        show_screen(SCREEN_FACE);  // Then to FACE
    }
}
