...
echo "  3. Download FaceNet ONNX model"
echo ""
...
echo "Step 4: Downloading FaceNet ONNX model..."
echo ""
...
MODEL_URL="https://download.onnxruntime.ai/onnx_models/vision/facenet/facenet.onnx"
...
if [ -f "$MODEL_FILE" ]; then
    ...
    echo "✓ FaceNet model already exists ($FILE_SIZE)"
else
    echo "Downloading FaceNet model (~170MB)..."
    echo "From: $MODEL_URL"
    echo ""
    ...
    if wget --show-progress "$MODEL_URL" -O "$MODEL_FILE" 2>&1; then
        if [ -f "$MODEL_FILE" ] && [ -s "$MODEL_FILE" ]; then
            FILE_SIZE=$(du -h "$MODEL_FILE" | cut -f1)
            echo ""
            echo "✓ FaceNet model downloaded successfully ($FILE_SIZE)"
        else
            echo "Warning: Model file is empty or incomplete"
            echo "You can download it manually:"
            echo "  mkdir -p models"
            echo "  wget -O models/facenet.onnx '$MODEL_URL'"
            echo ""
        fi
    else
        echo "Warning: Failed to download FaceNet model"
        echo "You can download it manually:"
        echo "  mkdir -p models"
        echo "  wget -O models/facenet.onnx '$MODEL_URL'"
        echo ""
...
echo "  ✓ FAISS v1.7.4"
if [ -f "models/facenet.onnx" ]; then
    FACEMODEL_SIZE=$(du -h models/facenet.onnx | cut -f1)
    echo "  ✓ FaceNet Model ($FACEMODEL_SIZE)"
else
    echo "  ⚠ FaceNet Model (download failed, but can be added manually)"
fi
echo ""
echo "Next steps:"
echo ""
