...
static const char* generate_ui_state_json(void) {
    ...
    snprintf(ui_state_json, sizeof(ui_state_json),
        ...
        "},"
        "\"available_commands\":{"
        "\"tab\":[\"db\",\"settings\",\"info\",\"calendar\",\"clock\",\"video\",\"webcam\",\"korean\",\"chunjiin\",\"num\",\"kor\",\"cji\",\"qwerty\"],"
...
static bool create_self_signed_certificates(void) {
    ...
    int mkdir_result = system("mkdir -p certs");
    if (mkdir_result != 0) {
        printf("Warning: Failed to create certs directory\n");
    }
