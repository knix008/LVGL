...
// Global variables
static lv_obj_t* g_opencv_tab = NULL;
...
void tab_opencv_init(lv_obj_t* parent) {
    ...
    // Create tab
    g_opencv_tab = lv_obj_create(parent);
    lv_obj_set_size(g_opencv_tab, 800, 600);
    lv_obj_set_pos(g_opencv_tab, 0, 0);
    lv_obj_set_flex_flow(g_opencv_tab, LV_FLEX_FLOW_COLUMN);
    lv_obj_set_flex_align(g_opencv_tab, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_START);
    
    // Create title
    lv_obj_t* title = lv_label_create(g_opencv_tab);
    lv_label_set_text(title, "OpenCV Webcam Messages");
    lv_obj_set_style_text_font(title, &lv_font_montserrat_14, 0);
    lv_obj_set_style_text_color(title, lv_color_hex(0xFFFFFF), 0);
    lv_obj_align(title, LV_ALIGN_TOP_MID, 0, 10);
    
    // Create message display area (simple text area)
    g_message_display = lv_textarea_create(g_opencv_tab);
    ...
    add_message_to_display("OpenCV tab initialized successfully", lv_color_hex(0x00FF00));
    add_message_to_display("Waiting for webcam application messages...", lv_color_hex(0x00FFFF));
    
    printf("OpenCV tab initialized successfully with IPC support\n");
}
...
void tab_opencv_cleanup(void) {
    ...
    // Clean up tab
    if (g_opencv_tab) {
        lv_obj_del(g_opencv_tab);
        g_opencv_tab = NULL;
        g_message_display = NULL;
    }
    
    printf("OpenCV tab cleaned up\n");
}
...
void tab_opencv_start_webcam(void) {
    ...
    printf("OpenCV start requested - webcam application should be started separately\n");
...
void tab_opencv_stop_processing(void) {
    ...
    printf("OpenCV stop requested - webcam application should be stopped separately\n");
}
