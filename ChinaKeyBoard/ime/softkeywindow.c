...
#define SFKB_CONFIG_FILE  "etc/sfkb.cfg"
#define SCREEN_DEFAULT_W 		240
#define SCREEN_DEFAULT_H 		320
#define SKB_WIN_DEFAULT_W	  	240
#define SKB_WIN_DEFAULT_H  		260
#define SFKB_NUM 5

static int gSfkbType = MGIN_SKB9IME;
...
extern int get_kpd_en_bitmap (HDC hdc, PBITMAP* pbmp, int id);
static void ChangeKBDFun(HWND hWnd, SOFTKBD_DATA* pdata, int ime_mode)
{
	...
	switch(mode)
	{
		...
		{
			y = 3;
			if ((MGIN_SKB26IME == gSfkbType) && (y == pdata->current_board_idx))
			{
				y = 4;
			}
			break;
	...
	if((0 == (pdata->current_board_idx)) && (MGIN_SKB26IME == gSfkbType))
	{
		get_kpd_en_bitmap(HDC_SCREEN, (PBITMAP) (&pdata->keyboard->data), 0);
	}
...
static key_board_t* init_keyboard9_data(HWND hWnd)
{
...
//初始化26键软键盘
static key_board_t *init_keyboard26_data(HWND hWnd)
{
	/* for english key board */
	if (!(keyboard[0] = (key_board_t*) calloc(1, sizeof(key_board_t))))
	{
	...
	if (-1 == init_en_keyboard(handle, hWnd, keyboard[0]))//初始化26键英文字母键盘
	{
		_MY_PRINTF("error for initalize En-keyboard.\n");
		return NULL;
	}
	...
	if (-1 == init_py_keyboard(handle, hWnd, keyboard[1]))//初始化26键拼音键盘
	{
		_MY_PRINTF("error for initalize PinYin-keyboard.\n");
		return NULL;
	}
	...
	if (-1 == init_num_keypad(handle, hWnd, keyboard[2]))//初始化9键数字键盘
	{
		_MY_PRINTF("error for initalize num-keyboard.\n");
		return NULL;
	}
	...
	if (-1 == init_num_keyboard(handle, hWnd, keyboard[3]))//初始化26键数字键盘
	{
		_MY_PRINTF("error for initalize num-keyboard.\n");
		return NULL;
	}
	...
	if (-1 == init_punct_keyboard(handle, hWnd, keyboard[4]))//初始化26字符键字母键盘
	{
		_MY_PRINTF("error for initalize Puctuation-keyboard.\n");
		return NULL;
	}
	...
	/* for Korean keyboard */
	if (!(keyboard[5] = (key_board_t*) calloc(1, sizeof(key_board_t))))
	{
		_MY_PRINTF("calloc keyboard data for Korean failed\n");
		return NULL;
	}
	if (-1 == init_korean_keyboard(handle, hWnd, keyboard[5]))//초기화 한국어 키보드
	{
		_MY_PRINTF("error for initialize Korean-keyboard.\n");
		return NULL;
	}
...
static destroy_key_win (void)
{
	...
	if (MGIN_SKB26IME == gSfkbType)
	{
		...
		if(NULL != keyboard[1])
	    {
			destroy_py_keyboard (keyboard[1]);
		    free(keyboard[1]);		
			keyboard[1] = NULL;
		}
		if(NULL != keyboard[2])
		...
		if(NULL != keyboard[3])
	    {
			destroy_num_keyboard (keyboard[3]);
		    free(keyboard[3]);		
			keyboard[3] = NULL;
		}
		if(NULL != keyboard[4])
	    {
		    destroy_punct_keyboard (keyboard[4]);
		    free(keyboard[4]);		
			keyboard[4] = NULL;
		}
		if(NULL != keyboard[5])
	    {
		    destroy_korean_keyboard (keyboard[5]);
		    free(keyboard[5]);		
			keyboard[5] = NULL;
		}
	}
	else if (MGIN_SKB9IME == gSfkbType)
	{
		//FIXME:
		if(NULL != keyboard[0])
		{
...
//	printf("SoftKeyWinProc===[%d]====\n", gSfkbType);
//	if(MSG_IDLE != message)
//		printfmy("caona [%s]--- msg=0x%x, %d, w=%d, l=%d \n", __func__, message, message, wParam, lParam);
	...
	switch (message)
	{
		...
		case MSG_CREATE:
		{
		   // printf("softkeyboard creat========[%d]=======\n", gSfkbType);
			...
			pdata->skb_ime = gSfkbType;
			if (MGIN_SKB26IME == gSfkbType)
			{
				if (NULL == (pdata->keyboard = init_keyboard26_data(hWnd)))
				{
			...
			{
				if (NULL == (pdata->keyboard = init_keyboard9_data(hWnd)))
				{
				//	_MY_PRINTF("Soft Key Window init failed\n");
					return -1;
				}
...
#ifdef KBD_TOOLTIP
			pdata->tooltip_win = CreateToolTip(hWnd);//创建输入提示框
#endif
		...
		case MSG_NCMOUSEMOVE:
		{
			...
			switch (pdata->keyboard->proceed_msg(pdata->keyboard, hWnd, message, wParam, lParam))
			{
				...
				default:
				{
					...
					if (MSG_LBUTTONDOWN == message)
					{
						...
						//printf("curtime = %u   char = %u \n", curtime, curtime - stime);
						if(curtime - stime > 50) ///500ms
						{
							stime = curtime;
#if defined(_MGRM_PROCESSES) && (MINIGUI_MAJOR_VERSION > 1) && !defined(_STAND_ALONE)
			...
			//printfmy("-key--message : %x,  ----end", message);
			return 0;
		}
		case MSG_PAINT:
		{
			...
			EndPaint(hWnd, hdc);
			//printfmy("-MSG_PAINT--message : %x,  ----end", message);
			return 0;
		}
		case MSG_ERASEBKGND:
		{
			...
			//printfmy("-MSG_ERASEBKGND--message : %x,  ----end", message);
			return 0;
		}
		case MSG_CLOSE:
		{
			...
			//printfmy("-MSG_CLOSE--message : %x,  ----end", message);
			return 0;
		}
	}
	ret = DefaultIMEWinProc(hWnd, message, wParam, lParam);
	//printfmy("---message : %x,  ----end", message);
	return ret;
}
...
static void init_createinfo (PMAINWINCREATE pCreateInfo)
{
    ...
    pCreateInfo->lx = 0; 
    pCreateInfo->ty = gScreen_h;
    pCreateInfo->rx = skb_win_w;
    pCreateInfo->by = gScreen_h;
...
//		printf("\033[4;32m env init error:%d \033[0m\n",FindMigCfgRectValue(handle,"RECT_EN_KEY_1"));
		printf("Softkeyboard InitMigConfig is NULL and use defaule config \n");
	}
	...
	{
		gScreen_h = SCREEN_DEFAULT_H;
	}

	if ((NULL != handle) && (valueCfg = FindMigCfgIntValue(handle, "SCREEN_W")) >= 0)
	{
		gScreen_w = valueCfg;
	}
	else
	{
		gScreen_w = SCREEN_DEFAULT_W;
	}

	if ((NULL != handle) && (valueCfg = FindMigCfgIntValue(handle, "SKBIME")) >= 0 && (9 == valueCfg || 26 == valueCfg))
	{
		if(26 == valueCfg)
		{
			gSfkbType = MGIN_SKB26IME;
			load_keyboard_bitmap(MGIN_SKB26IME);
		}
		else
		{
			gSfkbType = MGIN_SKB9IME;
			load_keyboard_bitmap(MGIN_SKB9IME);
		} 
	}
	else
	{
	...
	printf("\033[4;32m libsoftkeywin.so Create SoftKeyboardIme successfull SKBIME=%d getLCDSize=%d, gScreen_w=%d, gScreen_h=%d\033[0m\n", valueCfg, getLCDSize(), gScreen_w, gScreen_h);
...
HWND CreateSoftKeypad (int sfkbType,void (*cb)(BOOL IsShown))
{
	op_cb = cb; 
	gSfkbType = sfkbType;
...
void HideSoftKeyWindow(HWND hWnd)
{
	...
	SOFTKBD_DATA* pdata = (SOFTKBD_DATA*) GetWindowAdditionalData(hWnd);
	if(!pdata)
		return ;
