...
bool ImageLoader::load_from_frame(const cv::Mat& frame, ImageData& image_data) {
    if (frame.empty()) {
        std::cerr << "Error: Empty frame provided" << std::endl;
        return false;
    }

    // Convert to RGB if necessary (Camera frame is BGR by default from OpenCV)
    cv::Mat rgb_image = to_rgb(frame);

    image_data.mat = rgb_image;
    image_data.file_path = "[Camera Frame]";
    image_data.faces.clear();

    std::cout << "Frame loaded from camera successfully"
              << " (" << rgb_image.cols << "x" << rgb_image.rows << ")" << std::endl;
