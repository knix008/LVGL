...
void create_info_tab(lv_obj_t * parent) {
    ...
    lv_image_set_src(logo_img, "A:../assets/IntellivixSmall.png");
    
    // Check if the image loaded successfully
    const void* src = lv_image_get_src(logo_img);
    if (src == NULL) {
        printf("Info tab: PNG failed to load: A:../assets/IntellvixSmall.png\n");
        printf("Info tab: PNG source is NULL\n");
    } else {
        printf("Info tab: PNG loaded successfully: A:../assets/IntellvixSmall.png\n");
        printf("Info tab: PNG source pointer: %p\n", src);
    }
