...
void lv_example_tab_menu(void) {
    ...
    // Create tabs
    lv_obj_t * tab1 = lv_tabview_add_tab(tabview, "NUM"); // Number keypad
    lv_obj_t * tab2 = lv_tabview_add_tab(tabview, "KOR"); // Korean input
    lv_obj_t * tab3 = lv_tabview_add_tab(tabview, "CJI"); // ChunJiIn input
    lv_obj_t * tab4 = lv_tabview_add_tab(tabview, "QWERTY"); // QWERTY keyboard
    lv_obj_t * tab5 = lv_tabview_add_tab(tabview, "Info");
    
    // Create tab contents using separate functions
    create_number_tab(tab1);
    create_korean_tab(tab2);
    create_chunjiin_tab(tab3);
    create_qwerty_tab(tab4);
    create_info_tab(tab5);
}